...
public class Batalha extends JFrame implements ActionListener {
	...
	private void acaoAtacar(int atacando, int atacado){
		...
		labelFeedback.setText(maqEqp.get(atacado).sofrerDano(reducao,usuEqp.get(atacando)));
	}
	...
	private void verificaVidaUsu(){
		...
		if(usuEqp.get(0).getVidaParcial() <=0 && usuEqp.get(0).isMorto() == false){
		...
		else if(usuEqp.get(1).getVidaParcial() <=0 && usuEqp.get(1).isMorto() == false){
		...
		else if(usuEqp.get(2).getVidaParcial() <=0 && usuEqp.get(2).isMorto() == false){
